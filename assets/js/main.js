...
const paletteButton = document.getElementById('color-button'); // Palette button
let currentHue = localStorage.getItem('--hue-color'); // Default Hue Color (matches your CSS variable)

paletteButton.addEventListener('click', () => {
    // Cycle through a few predefined colors (e.g., 200, 250, 340, etc.)
    currentHue += 20; // Increment hue
    if (currentHue > 360) currentHue = 0; // Reset hue if it exceeds 360

    // Update the --hue-color CSS variable
    document.documentElement.style.setProperty('--hue-color', currentHue);

    // Save the current hue to localStorage to persist the selection
    localStorage.setItem('selected-hue', currentHue);
});

// On page load, check for previously saved hue
const savedHue = localStorage.getItem('selected-hue');
if (savedHue) {
    currentHue = parseInt(savedHue, 10); // Restore saved hue
    document.documentElement.style.setProperty('--hue-color', currentHue);
}
